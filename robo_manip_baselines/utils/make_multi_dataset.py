...
def load_data(in_dir, task_names, skip, resized_img_size, nproc):
    ...
    _tasks = tasks + [
        {task_name for task_name in task_names if task_name in in_file_name}
...
if __name__ == "__main__":
    ...
    save_arr(
        args.out_dir, "train/tasks.npy", tasks[train_idx_list].astype(str), args.quiet
    )
    save_arr(
        args.out_dir,
        "test/masks.npy",
    ...
    save_arr(
        args.out_dir, "test/tasks.npy", tasks[test_idx_list].astype(str), args.quiet
    )
