...
class TrainAct(object):
    ...
    def setupArgs(self):
        ...
        parser.add_argument("--ckpt_dir", action="store", type=str, help="ckpt_dir", required=False, default="./log/")
    ...
    def train_bc(self):
        ...
        for epoch in tqdm(range(self.args.num_epochs)):
            ...
            if epoch % 100 == 0:
                ckpt_path = os.path.join(self.args.ckpt_dir, f"policy_epoch_{epoch}_seed_{self.args.seed}.ckpt")
        ...
        best_epoch, min_val_loss, best_state_dict = best_ckpt_info
        ckpt_path = os.path.join(self.args.ckpt_dir, f"policy_epoch_{best_epoch}_seed_{self.args.seed}.ckpt")
        torch.save(best_state_dict, ckpt_path)
        print(f"[TrainAct] Training finished: seed {self.args.seed}, val loss {min_val_loss:.3f} at epoch {best_epoch}")
    ...
    def forward_pass(self, data):
        ...
        image_data, joint_data, action_data, is_pad = image_data.cuda(), joint_data.cuda(), action_data.cuda(), is_pad.cuda()
        return self.policy(joint_data, image_data, action_data, is_pad) # TODO remove None

    def plot_history(self, train_history, validation_history, epoch):
        # save training curves
        for key in train_history[0]:
            plot_path = os.path.join(self.args.ckpt_dir, f"train_val_{key}_seed_{self.args.seed}.png")
            plt.figure()
