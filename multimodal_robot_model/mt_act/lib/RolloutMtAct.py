...
class RolloutMtAct(RolloutBase):
    def setupArgs(self, parser=None):
        ...
        parser.add_argument("--task_name", choices=TASKS, action="store", type=str, help="task name", required=True)
        ...
        # Add dummy arguments
        parser.add_argument("--policy_class", choices=["ACT"], type=str,
                            help="Do not set this manually as it is a dummy argument.", required=True)
        parser.add_argument("--num_epochs", choices=[-1], type=int,
                            help="Do not set this manually as it is a dummy argument.", required=True)
    ...
    def inferPolicy(self):
        ...
        # Infer
        all_actions = self.policy(joint_input, front_image_input, task_emb=self.task_emb)[0]
    ...
    def drawPlot(self):
        ...
        for layer_idx, layer in enumerate(self.policy.model.transformer.encoder.layers):
            if layer.self_attn.correlation_mat is None:
                continue
            self.ax[1, layer_idx].imshow(layer.self_attn.correlation_mat[3:, 1].reshape((15, 20)))
